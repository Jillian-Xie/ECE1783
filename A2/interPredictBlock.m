...
for indexRefFrame = 1:numRefFrames
    for mvX = -r:r
        for mvY = -r:r
            ...
            if checkFrameBoundary(refWidthPixelIndex, refHeightPixelIndex, blockSize, refFrame(:,:,indexRefFrame)) == 1
                refBlock = getBlockContent(widthBlockIndex, heightBlockIndex, blockSize, refFrame(:,:,indexRefFrame), mvX, mvY);
